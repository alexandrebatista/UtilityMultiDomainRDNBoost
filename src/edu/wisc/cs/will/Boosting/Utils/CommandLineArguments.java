...
public class CommandLineArguments {
    ...
    private static final String focalLossAlpha = "focalLossAlpha";
    private double focalLossAlphaVal = 0.66;

    private static final String focalLossGamma = "focalLossGamma";
    private double focalLossGammaVal = 2;

    private static final String boxCoxLambda = "boxCoxLambda";
    private double boxCoxLambdaVal = 0;

    private static final String maxTreeDepthInLiterals = "maxTreeDepthInLiterals";
    private int maxTreeDepthInLiteralsVal = -1;

    private static final String rdnVersion = "rdnVersion";
    ...
    public String getAnyStringForModEquals() {
        if (doInferenceIfModNequalsThis < 0) {
            return "";
        }
    ...
    public boolean parseArgs(String[] args) {
        for (int i = 0; i < args.length; i++) {
            ...
            if (argMatches(args[i], "noLockFiles")) {
                ...
                switch (hundsDigit) {
                    case 0:
                        target += (thousDigit <= 0 ? "gameLoser_NFLTeamNFLGameNFLGame" : "scoringEventTD_NFLTeamNFLGameScore");
                        break;
                    case 1:
                        target += (thousDigit <= 0 ? "gameWinner_NFLTeamNFLGameNFLGame" : "scoringEventFG_NFLTeamNFLGameScore");
                        break;
                    case 2:
                        target += "homeTeamInGame_NFLTeamNFLGame";
                        break;        // I. e., should do 1300 runs (0 through 1199).
                    case 3:
                        target += "awayTeamInGame_NFLTeamNFLGame";
                        break;
                    case 4:
                        target += "gameDate_DateNFLGame";
                        break;
                    case 5:
                        target += "NFLGame";
                        break;
                    ...
                    case 8:
                        ...
                        break;
                    case 9:
                        target += "teamFinalScore_ScoreNFLTeamNFLGame";
                        break;
                }
                ...
                if (modelDirVal == null) {
                    modelDirVal = "/u/shavlikgroup/people/Jude/MRtestbedResults/NFL/";
                }
                if (resultsDirVal == null) {
                    resultsDirVal = modelDirVal;
                }
                if (testDirVal == null) {
                    testDirVal = "/u/shavlikgroup/projects/MachineReading/MRtestbeds/NFL/";
                }
                ...
                if (RunBoostedRDN.nameOfCurrentModel == null) {
                    RunBoostedRDN.nameOfCurrentModel = "Run1";
                }
            ...
            if (argMatches(args[i], "condorJWSlearn")) { // Special-purpose code for JWS' experiments.  Currently only evaluates learned bRDN models on testsets.
                ...
                if (fold > 4) {
                    System.exit(0);
                } // Can we use these for something? USE  TO SUPPORT TWO ALGO VARIANTS?  IF SO, CREATE 'marker' here (rather than in the calling script).

                switch (tensDigit) { // Could rewrite this to only need 60 Condor jobs ...
                    case 0:
                        target += (hundsDigit <= 0 ? "gameLoser_NFLTeamNFLGame" : "scoringEventTD_NFLTeamNFLGameScore");
                        break;
                    case 1:
                        target += (hundsDigit <= 0 ? "gameWinner_NFLTeamNFLGame" : "scoringEventFG_NFLTeamNFLGameScore");
                        break;
                    case 2:
                        target += "homeTeamInGame_NFLTeamNFLGame";
                        break;        // I. e., should do 120 runs (0 through 119).
                    case 3:
                        target += "awayTeamInGame_NFLTeamNFLGame";
                        break;
                    case 4:
                ...
                if (trainDirVal == null) {
                    trainDirVal = "/u/shavlikgroup/projects/MachineReading/MRtestbeds/NFL/";
                }
                modelDirVal += target + "/trainf" + fold + "/models" + experimentID + "/";
            ...
            if (argMatches(args[i], focalLossAlpha)) {
                focalLossAlphaVal=Double.parseDouble(args[++i]);
                continue;
            }
            if (argMatches(args[i], focalLossGamma)) {
                focalLossGammaVal=Double.parseDouble(args[++i]);
                continue;
            }
            if (argMatches(args[i], boxCoxLambda)) {
                boxCoxLambdaVal=Double.parseDouble(args[++i]);
                continue;
            }
            if (argMatches(args[i], maxTreeDepthInLiterals)) {
                maxTreeDepthInLiteralsVal=Integer.parseInt(args[++i]);
                continue;
            }
            if (argMatches(args[i], rdnVersion)) {
                rdnVersionVal=args[++i];
                continue;
            }
    ...
    public void setTrainDirVal(String trainDirVal) {
        checked_trainDirVal = true;
        if (!(trainDirVal.endsWith("/") || trainDirVal.endsWith("\\"))) {
            trainDirVal += "/";
        }
    ...
    public void setTestDirVal(String testDirVal) {
        checked_testDirVal = true;
        if (!(testDirVal.endsWith("/") || testDirVal.endsWith("\\"))) {
            testDirVal += "/";
        }
    ...
    public void setModelDirVal(String modelDirVal) {
        checked_modelDirVal = true;
        if (!(modelDirVal.endsWith("/") || modelDirVal.endsWith("\\"))) {
            modelDirVal += "/";
        }
    ...
    public void setResultsDirVal(String resultsDirVal) {
        checked_resultsDirVal = true;
        if (!(resultsDirVal.endsWith("/") || resultsDirVal.endsWith("\\"))) {
            resultsDirVal += "/";
        }
    ...
    public String getExtraMarkerForFiles(boolean includeTestSkew) {
        String result = "_";
        if (stringForTestsetPos != null) {
            result += stringForTestsetPos + "_";
        }
        if (stringForTestsetNeg != null) {
            result += stringForTestsetNeg + "_";
        }
        result += getAnyStringForModEquals();
        if (maxLiteralsInAnInteriorNodeVal >= 0) {
            result += "Lits" + maxLiteralsInAnInteriorNodeVal;
        }
        if (maxTreesVal >= 0) {
            result += "Trees" + maxTreesVal;
        }
        if (sampleNegsToPosRatioVal >= 0) {
            result += "Skew" + (int) sampleNegsToPosRatioVal;
        }
        if (includeTestSkew &&
                testNegsToPosRatioVal >= 0) {
            result += "TestSkew" + (int) testNegsToPosRatioVal;
        }
        return result;
    }
    ...
    public double getFocalLossAlphaVal() {
        return focalLossAlphaVal;
    }

    public void getFocalLossAlphaVal(double alpha) {
        focalLossAlphaVal = alpha;
    }

    public double getFocalLossGammaVal() {
        return focalLossGammaVal;
    }

    public void setFocalLossGammaVal(double gamma) {
        focalLossGammaVal = gamma;
    }

    public double getBoxCoxLambdaVal() {
        return boxCoxLambdaVal;
    }

    public void setBoxCoxLambdaVal(double lambda) {
        boxCoxLambdaVal = lambda;
    }

    public int getMaxTreeDepthInLiteralsVal() {
        return maxTreeDepthInLiteralsVal;
    }

    public void setMaxTreeDepthInLiteralsVal(int literalsVal) {
        maxTreeDepthInLiteralsVal = literalsVal;
    }

    public String getRdnVersionVal() {
        return rdnVersionVal;
    }

    public void setRdnVersionVal(String rdnVersionVal) {
